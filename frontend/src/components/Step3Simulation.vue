  ...
  <div class="simulation-panel">
    ...
    <div class="control-bar">
      <div class="status-group">
        <!-- Progression de la plateforme Info Plaza (Twitter) -->
        ...
        <!-- Progression de la plateforme Topic Community (Reddit) -->
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? 'Démarrage...' : 'Générer le rapport de résultats' }} 
              ...
              <div class="card-body">
                ...
                <template v-if="action.action_type === 'REPOST'">
                  <div class="repost-info">
                    ...
                    <span class="repost-label">Repartagé par @{{ action.action_args?.original_author_name || 'Utilisateur' }}</span>
                  </div>
                ...
                <template v-if="action.action_type === 'LIKE_POST'">
                  <div class="like-info">
                    ...
                    <span class="like-label">A aimé le post de @{{ action.action_args?.post_author_name || 'Utilisateur' }}</span>
                  </div>
                ...
                <template v-if="action.action_type === 'UPVOTE_POST' || action.action_type === 'DOWNVOTE_POST'">
                  <div class="vote-info">
                    ...
                    <span class="vote-label">{{ action.action_type === 'UPVOTE_POST' ? 'A voté pour' : 'A voté contre' }} le post</span>
                  </div>
    ...
    <div class="system-logs">
      <div class="log-header">
        <span class="log-title">SURVEILLANCE DE LA SIMULATION</span>
...
const doStartSimulation = async () => {
  ...
  startError.value = null
  addLog('Démarrage de la simulation parallèle bi-plateforme...')
  ...
  try {
    ...
    if (props.maxRounds) {
      ...
      addLog(`Nombre maximum de tours défini : ${props.maxRounds}`)
    }
    
    addLog('Mode de mise à jour dynamique du graphe activé')
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Anciens rapports de simulation nettoyés, redémarrage du processus')
      }
      addLog('✓ Moteur de simulation démarré avec succès')
    ...
    addLog(`✗ Exception lors du démarrage : ${err.message}`)
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const handleStopSimulation = async () => {
  ...
  try {
    ...
    addLog(`Exception lors de l'arrêt : ${err.message}`)
  } finally {
    isStopping.value = false
  }
}
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ Toutes les simulations de plateforme détectées comme terminées')
        }
    ...
    console.warn('Échec de la récupération du statut d\'exécution :', err)
  }
}
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    console.warn('Échec de la récupération du statut détaillé :', err)
  }
}
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('Demande de génération de rapport envoyée, veuillez patienter...')
    return
  }
  
  isGeneratingReport.value = true
  addLog('Démarrage de la génération de rapport...')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ Tâche de génération de rapport démarrée : ${reportId}`)
      
      // 跳转到报告页面
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(`✗ Échec de la génération de rapport : ${res.error || 'Erreur inconnue'}`)
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`✗ Exception lors de la génération de rapport : ${err.message}`)
    isGeneratingReport.value = false
  }
}
...
onMounted(() => {
  addLog('Initialisation de l\'étape 3 : Exécution de la simulation')
  if (props.simulationId) {
    doStartSimulation()
  }
})
