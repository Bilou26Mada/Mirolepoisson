        ...
        <div class="action-bar-header">
          ...
          <div class="action-bar-tabs">
            ...
            >
              ...
              <span>Discuter avec l'Agent de Rapport</span>
            </button>
            <div class="agent-dropdown" v-if="profiles.length > 0">
              ...
              >
                ...
                <span>{{ selectedAgent ? selectedAgent.username : 'Discuter avec n\'importe qui' }}</span>
                <svg class="dropdown-arrow" :class="{ open: showAgentDropdown }" viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2">
                  <polyline points="6 9 12 15 18 9"></polyline>
                </svg>
              </button>
              <div v-if="showAgentDropdown" class="dropdown-menu">
                <div class="dropdown-header">Sélectionner l'interlocuteur</div>
                <div 
            ...
            >
              ...
              <span>Diffuser un questionnaire dans le monde</span>
            </button>
          </div>
        </div>
        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            <div class="tools-card-header">
              ...
              <div class="tools-card-info">
                ...
                <div class="tools-card-subtitle">Version de chat rapide de l'agent de rapport, peut appeler 4 outils spécialisés, possède la mémoire complète de MiroFish</div>
              </div>
              ...
              <div class="tools-grid">
                <div class="tool-item tool-purple">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">InsightForge : Attribution profonde</div>
                    <div class="tool-desc">Aligne les données source réelles avec l'état de l'environnement simulé, combine les mécanismes de mémoire globale/locale pour fournir une analyse d'attribution profonde.</div>
                  </div>
                </div>
                <div class="tool-item tool-blue">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">Algorithme de parcours en largeur basé sur les graphes, reconstruit les chemins de propagation d'événements, capture la topologie des flux d'informations.</div>
                  </div>
                </div>
                <div class="tool-item tool-orange">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">Interface de requête instantanée basée sur GraphRAG, optimise l'efficacité de l'indexation pour l'extraction rapide d'attributs de nœuds et de faits discrets.</div>
                  </div>
                </div>
                <div class="tool-item tool-green">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">Entretien autonome, capable de mener des dialogues multi-tours en parallèle avec les individus du monde simulé, collecte des données d'opinion non structurées et des états psychologiques.</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="chat-messages" ref="chatMessages">
            <div v-if="chatHistory.length === 0" class="chat-empty">
              ...
              <p class="empty-text">
                {{ chatTarget === 'report_agent' ? 'Discutez avec l\'Agent de Rapport pour approfondir le contenu du rapport' : 'Discutez avec les individus simulés pour comprendre leurs points de vue' }}
              </p>
            </div>
            <div 
          ...
          <div class="survey-setup">
            <div class="setup-section">
              <div class="section-header">
                <span class="section-title">Sélectionner les sujets d'enquête</span>
                <span class="selection-count">Sélectionné {{ selectedAgents.size }} / {{ profiles.length }}</span>
              </div>
            ...
            <div class="setup-section">
              ...
              <textarea 
                ...
                class="survey-input"
                placeholder="Saisissez la question que vous souhaitez poser à tous les sujets sélectionnés..."
                rows="3"
              ></textarea>
            </div>
            ...
            >
              ...
              <span v-else>Diffuser le questionnaire</span>
            </button>
          </div>
...
const chatHistoryCache = ref({}) // Cache tous les enregistrements de dialogue : { 'report_agent': [], 'agent_0': [], 'agent_1': [], ... }
...
// Sauvegarder l'enregistrement actuel du dialogue dans le cache
...
const selectReportAgentChat = () => {
  ...
  // Récupérer l'enregistrement du dialogue de l'Agent de Rapport
...
const selectAgent = (agent, idx) => {
  ...
  // Récupérer l'enregistrement du dialogue de cet Agent
  chatHistory.value = chatHistoryCache.value[`agent_${idx}`] || []
  addLog(`Sélectionner l'interlocuteur : ${agent.username}`)
}
...
const sendToReportAgent = async (message) => {
  addLog(`Envoi à l'Agent de Rapport : ${message.substring(0, 50)}...`)
  ...
  if (res.success && res.data) {
    chatHistory.value.push({
      role: 'assistant',
      content: res.data.response || res.data.answer || 'Pas de réponse',
      timestamp: new Date().toISOString()
    })
...
const sendToAgent = async (message) => {
  ...
  addLog(`Envoi à ${selectedAgent.value.username} : ${message.substring(0, 50)}...`)
  ...
  if (chatHistory.value.length > 1) {
    const historyContext = chatHistory.value
      ...
      .slice(-6)
      .map(msg => `${msg.role === 'user' ? 'Interlocuteur' : 'Toi'} : ${msg.content}`)
      .join('\n')
    prompt = `Voici notre conversation précédente :\n${historyContext}\n\nMaintenant, ma nouvelle question est : ${message}`
  }
  ...
  if (res.success && res.data) {
    // Chemin de données correct : res.data.result.results est un dictionnaire d'objets
    // Format : {"twitter_0": {...}, "reddit_0": {...}} ou plateforme unique {"reddit_0": {...}}
    const resultData = res.data.result || res.data
    const resultsDict = resultData.results || resultData
    
    // Convertir le dictionnaire d'objets en tableau, donner la priorité à la réponse de la plateforme Reddit
    ...
    if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
      ...
      // Compatible avec le format tableau
    ...
    if (responseContent) {
      ...
      throw new Error('Pas de données de réponse')
    }
  } else {
    throw new Error(res.error || '请求失败')
  }
...
const submitSurvey = async () => {
  ...
  addLog(`Envoi du questionnaire à ${selectedAgents.value.size} sujets...`)
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      for (const interview of interviews) {
        ...
        // 优先使用 reddit 平台回复，其次 twitter
        let responseContent = 'Pas de réponse'
        ...
        if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
          ...
          if (agentResult) {
            responseContent = agentResult.response || agentResult.answer || 'Pas de réponse'
          }
        } else if (Array.isArray(resultsDict)) {
          // 兼容数组格式
          const matchedResult = resultsDict.find(r => r.agent_id === agentIdx)
          if (matchedResult) {
            responseContent = matchedResult.response || matchedResult.answer || 'Pas de réponse'
          }
        }
      ...
      addLog(`Reçu ${surveyResults.value.length} réponses`)
    } else {
      throw new Error(res.error || '请求失败')
    }
  } catch (err) {
    addLog(`Échec de l'envoi du questionnaire : ${err.message}`)
  } finally {
...
const loadReportData = async () => {
  ...
  try {
    ...
    addLog(`Échec du chargement du rapport : ${err.message}`)
  }
}
...
const loadAgentLogs = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog('Données du rapport chargées')
    }
  } catch (err) {
    addLog(`Échec du chargement des logs du rapport : ${err.message}`)
  }
}
...
const loadProfiles = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`${profiles.value.length} individus simulés chargés`)
    }
  } catch (err) {
    addLog(`Échec du chargement des individus simulés : ${err.message}`)
  }
}
...
// Lifecycle
onMounted(() => {
  addLog('Initialisation de l\'étape 5 : Interaction profonde')
  loadReportData()
  loadProfiles()
